 ...
 * @returns         object
 */
function getPayloadFromFS(options) {
    let { source_dir: sourceDir, filename } = options;
    sourceDir = sourceDir || process.env.CI_PROJECT_DIR;
...
module.exports = (pipeline, transport) => {
    let API = new APIService(pipeline.url(), pipeline.auth(), transport);

    let options = {}, payload = '';
    'payload_source batch_name batch_notes batch_packages batch_file source_dir'
        .split(' ')
        .forEach(name => {
        ...
        switch (options.payload_source) {
            case 'file':
                payload = getPayloadFromFS(options);
                break;
            case 'pipeline':
                payload = {
