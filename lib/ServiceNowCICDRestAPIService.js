...
function ServiceNowCICDRestAPIService(instance, auth, transport = null) {
    ...
    function batchInstall(url, payload) {
        return request(url, false, 'POST', payload)
            ...
            .then(async (resp) => {
                ...
                await extractBatchResults(resultsUrl).then(msg => msg && console.log("Status messages:", msg));
    ...
    function extractBatchResults(url) {
        return url && request(url).then((resp) => {
            ...
            return msg;
        }).catch(err => console.log("Fetch details error: ", err));
    }
    ...
    function appRepoInstall(options) {
        ...
        return request('app_repo/install', {fields: 'sys_id scope version base_app_version auto_upgrade_base_app', options}, 'POST')
            .then(resp => getProgress(resp))
            .catch(err => Promise.reject(err.errorMessage))
    ...
    function appRepoPublish(options) {
        ...
        if (['autodetect','detect'].includes(options.version_format) && 
            options.is_app_customization === true && !options.sys_id) {
            return Promise.reject('Sys_id is not specified!');
        }
        if (options.scope && options.sys_id) {
            delete options.scope;
        }
